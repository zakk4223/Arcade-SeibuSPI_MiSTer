...
-- SLOP_COIN_AT / SLOP_START_AT: emulated seconds at which to hold Coin 1 and
-- 1 Player Start for a few frames. Needed for any screen that is not reachable
-- from attract -- the player select above all, which is where rfjet's missing
-- sprites are most obvious and which nothing in this suite has ever captured.
local COIN_AT  = tonumber(os.getenv("SLOP_COIN_AT")  or "0")
local START_AT = tonumber(os.getenv("SLOP_START_AT") or "0")

local function field(tag, name)
    local p = mach.ioport.ports[tag]
    return p and p.fields[name] or nil
end

if COIN_AT > 0 or START_AT > 0 then
    local coin  = field(":COIN", "Coin 1")
    local start = field(":SYSTEM", "1 Player Start")
    slop_input_timer = emu.register_periodic(function()
        local t = mach.time:as_double()
        -- Hold for ~0.15 s, which is comfortably more than the game's poll.
        if coin  then coin:set_value((t >= COIN_AT  and t < COIN_AT  + 0.15) and 1 or 0) end
        if start then start:set_value((t >= START_AT and t < START_AT + 0.15) and 1 or 0) end
        -- SLOP_TAP_AT: from this second on, tap P1 Button 1 every 3 s. Needed to
        -- walk past name entry and the briefing to reach the player select.
        local TAP = tonumber(os.getenv("SLOP_TAP_AT") or "0")
        local b1 = field(":INPUTS", "P1 Button 1")
        if b1 and TAP > 0 then
            b1:set_value((t >= TAP and ((t - TAP) % 3.0) < 0.12) and 1 or 0)
        end
    end)
end
