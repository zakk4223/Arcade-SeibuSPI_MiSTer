...
-- SLOP_AT: comma-separated emulated times to dump at, instead of waiting for a
-- human to press P. The pause path below still works; this just adds a way to
-- capture a scene reproducibly, which matters when the scene has to be the same
-- one on two different runs.
local AT = {}
for s in (os.getenv("SLOP_AT") or ""):gmatch("[^,]+") do AT[#AT+1] = tonumber(s) end
local at_i = 1

print("[slop] mame_probe armed. " ..
      (#AT > 0 and ("dumping at t = " .. (os.getenv("SLOP_AT") or "")) or
                   "Press P at the scene you want") .. "; state goes to " .. OUT)

emu.register_periodic(function()
    if at_i <= #AT and mach.time:as_double() >= AT[at_i] then
        at_i = at_i + 1
        dump()
        if at_i > #AT then mach:exit() end
    end
end)
